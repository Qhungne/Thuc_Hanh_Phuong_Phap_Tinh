function vehamnoisuy(x, y, result, x0, bieudo_2, phuongphap)
    ...
    for i = 1:length(xx)
        if strcmp(phuongphap, 'Lagrange')
            yy(i) = noisuyLagrange(x, y, xx(i));  % Tính giá trị nội suy tại xx(i)
        elseif strcmp(phuongphap, 'Newton')
            yy(i) = noisuyNewton(x, y, xx(i));  % Tính giá trị nội suy tại xx(i)
        end
    end
